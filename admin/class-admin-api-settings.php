 ...
 * @author Adil Soomro
 */
if ( ! class_exists( 'AdminApiSettings' ) ) {
	class AdminApiSettings
	{
		...
		public function __construct($plugin_name, $version)
		{
			...
			add_action('admin_menu', array($this, 'houzi_admin_api_add_plugin_page'));
	
			$this->settings = new AdminApiAdminSettings($this->plugin_name,$this->version);
			$this->eleven = new AdminApiElevenSettings($this->plugin_name,$this->version);
			$this->iap = new AdminApiIAPProductIds($this->plugin_name,$this->version);
			$this->notify = new AdminApiNotify($this->plugin_name, $this->version);
			$this->contacts = new AdminApiFormsSettings($this->plugin_name, $this->version);
		}
	
		public function houzi_admin_api_add_plugin_page()
		{
			add_menu_page(
				'Houzi Admin Api',
				// page_title
				...
				'houzi-admin-api',
				// menu_slug
				array($this, 'houzi_admin_api_create_admin_page'), // function
				HOUZI_ADMIN_API_IMAGE . 'houzi-logo.svg',
				// icon_url
				80 // position
			);
		}
	
		public function houzi_admin_api_create_admin_page()
		{
			$this->houzi_admin_api_options = get_option('houzi_admin_api_options');
			?>
	
			<div class="wrap">
				<h2>Houzi Admin Api</h2>
				<p>Enhanced Admin Api for mobile apps.
					...
					<br />Ver:
					<?php echo HOUZI_ADMIN_API_VERSION ?>
				</p>
				<?php settings_errors();
	
				$is_elevened = $this->is_elevened();
				$active_tab = 'settings'; //$is_elevened ? 'settings' : 'p_code';
