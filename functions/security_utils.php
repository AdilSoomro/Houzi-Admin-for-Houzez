...
add_action( 'rest_api_init', function () {
    register_rest_route( 'houzez-admin-api/v1', '/create-nonce', array(
    'methods' => 'POST',
    'callback' => 'create_nonceFunction',
    'permission_callback' => '__return_true'
  ));
});
function create_nonceFunction($request) {
    ...
    $saved_app_secret = get_saved_app_secretFunction();
...
function nonce_security_enabledFunction() {
...
function get_saved_app_secretFunction() {
    return get_option( 'houzi_admin_app_secret', '' );
}
function create_nonce_or_throw_errorFunction($request_var, $nonce_var) {
    if (isset($_REQUEST[$request_var]) && !empty($_REQUEST[$request_var])) {
        return true;
    }
    if (nonce_security_enabledFunction()) {
