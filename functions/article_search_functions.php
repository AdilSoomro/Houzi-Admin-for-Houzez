...
// houzez-admin-api/v1/search-article
add_action( 'rest_api_init', function () {
  register_rest_route( 'houzez-admin-api/v1', '/search-articles', array(
    'methods' => 'POST',
    'callback' => 'search_articlesFunction',
    'permission_callback' => '__return_true'
  ));
  register_rest_route( 'houzez-admin-api/v1', '/article-comments', array(
    'methods' => 'POST',
    'callback' => 'list_article_commentsFunction',
    'permission_callback' => '__return_true'
  ));
  register_rest_route( 'houzez-admin-api/v1', '/add-comment', array(
    'methods' => 'POST',
    'callback' => 'add_commentFunction',
    'permission_callback' => '__return_true'
  ));

  register_rest_route( 'houzez-admin-api/v1', '/all-categories', array(
    'methods' => 'GET',
    'callback' => 'all_categoriesFunction',
    'permission_callback' => '__return_true'
  ));

  register_rest_route( 'houzez-admin-api/v1', '/all-tags', array(
    'methods' => 'GET',
    'callback' => 'all_tagsFunction',
    'permission_callback' => '__return_true'
  ));

});

function search_articlesFunction() {
    
    $query_args = setup_article_search_queryFunction();
    query_article_and_send_jsonFunction($query_args);
}

function query_article_and_send_jsonFunction($query_actual) {
    ...
    while( $query_args->have_posts() ):
        ...
        array_push($articles, article_nodeFunction($article) );
...
function setup_article_search_queryFunction() {
    $meta_query = array();
    $tax_query = array();
    $date_query = array();
    $allowed_html = array();
...
function article_nodeFunction($article){
    ...
    $article->categories = get_current_language_categoriesFunction($post_id);
    $article->tags = get_current_language_tagsFunction($post_id);
    $article->comment_count = get_comment_count($post_id);
    
    return $article;
}
function get_current_language_categoriesFunction($post_id) {
...
function get_current_language_tagsFunction($post_id) {
    $current_lang = apply_filters( 'wpml_current_language', "en" );
...
if( !function_exists('add_commentFunction') ) {
function add_commentFunction() {
    ...
    if (!create_nonce_or_throw_errorFunction('comment-security', 'comment-nonce')) {
        return;
    }
...
function list_article_commentsFunction() {
    $query_args = setup_comment_search_queryFunction();
    query_comment_and_send_jsonFunction($query_args);
}
function setup_comment_search_queryFunction() {
...
function query_comment_and_send_jsonFunction($query_actual) {
...
function all_categoriesFunction() {
