...
add_action('rest_api_init', function () {
  register_rest_route('houzez-admin-api/v1', '/touch-base', array(
    'methods' => 'GET',
    'callback' => 'getMetaDataFunction',
    'permission_callback' => '__return_true'
  ));
  register_rest_route('houzez-admin-api/v1', '/get-terms', array(
    'methods' => 'GET',
    'callback' => 'getTermsFunction',
    'permission_callback' => '__return_true'
  ));
  register_rest_route('houzez-admin-api/v1', '/houzi-setup-status', array(
    'methods' => 'POST',
    'callback' => 'houziSetupStatusFunction',
    'permission_callback' => '__return_true'
  ));
});

function getTermsFunction()
{
  if (!isset($_GET['term'])) {
  ...
  if (is_array($terms)) {
    foreach ($terms as $term):
      add_term_to_responseFunction($response, $term);
    endforeach;
  } else {
    $parent_slug = $_GET['parent_slug'] ?? null;
    add_term_to_responseFunction($response, $terms, $parent_slug);
  }
...
function get_base_currencyFunction()
{
  $base_currency_code = htf_get_base_currency();
  $combined_rates = combine_responseFunction();
...
function exchange_rate_currency_dataFunction()
{
...
function exchange_rates_currency_dataFunction()
{
  $currencies = Fcc_get_currencies();
  $base_currency = htf_get_base_currency();
  $rates = Fcc_get_exchange_rates($base_currency);
...
function combine_responseFunction()
{
  $list1 = exchange_rates_currency_dataFunction();
  $list2 = exchange_rate_currency_dataFunction();
  $combined = [];
...
function houzez_get_new_ratings_approval_settingFunction()
{
  return (bool) houzez_option('property_reviews_approved_by_admin');
}

function houzez_get_update_review_approval_settingFunction()
{
...
function getMetaDataFunction()
{
  ...
  $response['success'] = true;
  $response['version'] = HOUZI_ADMIN_API_VERSION;
  ...
  $response['new_ratings_approved_by_admin'] = houzez_get_new_ratings_approval_settingFunction();
  $response['update_review_approved'] = houzez_get_update_review_approval_settingFunction();
  ...
  if (houzez_option('multi_currency') == 1) {
    ...
    $response['base_currency'] = get_base_currencyFunction();
    $response['currency_rates'] = combine_responseFunction();
  }
  ...
  $options = get_option('houzi_admin_api_options'); // Array of All Options
  ...
  add_term_to_responseFunction($response, 'property_country');
  add_term_to_responseFunction($response, 'property_state');
  add_term_to_responseFunction($response, 'property_city');
  add_term_to_responseFunction($response, 'property_type');
  add_term_to_responseFunction($response, 'property_label');
  add_term_to_responseFunction($response, 'property_status');
  add_term_to_responseFunction($response, 'property_feature');
  ...
  add_custom_fields_to_responseFunction($response);
  add_roles_to_responseFunction($response);
...
/// Author: Ahmad Nasir
function add_custom_fields_to_responseFunction(&$response)
{
...
// 
// new code:
function add_term_to_responseFunction(&$response, $key, $parent_slug = null)
{
  if (!taxonomy_exists($key)) {
    $response[$key] = [];
    return;
...
function add_roles_to_responseFunction(&$response)
{
...
function houziSetupStatusFunction()
{
