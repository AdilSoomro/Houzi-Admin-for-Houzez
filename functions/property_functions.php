...
add_action( 'litespeed_init', function() {
    ...
    foreach ($exclude_url_list as $exclude_url) {
        if (strpos($_SERVER['REQUEST_URI'], $exclude_url) !== FALSE) {
            do_action( 'litespeed_control_set_nocache', 'no-cache for admin api' );
        }
    }
    ...
    foreach ($include_url_list as $include_url) {
        if (strpos($_SERVER['REQUEST_URI'], $exclude_url) !== FALSE) {
            do_action( 'litespeed_control_set_cacheable', 'cache for admin api' );
        }
    }

});

// houzez-admin-api/v1/search-properties
add_action( 'rest_api_init', function () {
    
    // register_rest_route( 'houzez-admin-api/v1', '/add-property', array(
    //   'methods' => 'POST',
    //   'callback' => 'addPropertyFunction',
    // ));

    register_rest_route( 'houzez-admin-api/v1', '/save-property', array(
        'methods' => 'POST',
        'callback' => 'addPropertyWithAuthFunction',
        'permission_callback' => '__return_true'
      ));
    
    register_rest_route( 'houzez-admin-api/v1', '/delete-property', array(
        'methods' => 'GET',
        'callback' => 'deletePropertyFunction',
        'permission_callback' => '__return_true'
      ));
      
	register_rest_route('houzez-admin-api/v1', '/check-user-verification', array(
    'methods' => 'GET',
    'callback' => 'checkUserVerificationFunction',
    'permission_callback' => function () {
        return is_user_logged_in();
    }
));
  
    register_rest_route( 'houzez-admin-api/v1', '/update-property', array(
      'methods' => 'POST',
      'callback' => 'addPropertyWithAuthFunction',
      'permission_callback' => '__return_true'
    ));

    // register_rest_route( 'houzez-admin-api/v1', '/upload-property-image', array(
    //     'methods' => 'POST',
    //     'callback' => 'uploadPropertyImage',
    //   ));
    register_rest_route( 'houzez-admin-api/v1', '/save-property-image', array(
        'methods' => 'POST',
        'callback' => 'uploadPropertyImageWithAuthFunction',
        'permission_callback' => '__return_true'
    ));
    register_rest_route( 'houzez-admin-api/v1', '/delete-property-image', array(
        'methods' => 'POST',
        'callback' => 'deleteImageForPropertyFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/like-property', array(
        'methods' => 'POST',
        'callback' => 'likePropertyFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/is-fav-property', array(
        'methods' => 'GET',
        'callback' => 'isFavPropertyFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/my-properties', array(
        'methods' => 'GET',
        'callback' => 'getMyPropertiesFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/property', array(
        'methods' => 'GET',
        'callback' => 'getPropertyFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/property-by-permalink', array(
        'methods' => 'GET',
        'callback' => 'getPropertyByPermalinkFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/print-pdf-property', array(
        'methods' => 'GET',
        'callback' => 'printPdfPropertyFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/property-by-meta-key', array(
        'methods' => 'GET',
        'callback' => 'getPropertyByMetaKeyFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/admin-approve-disapprove', array(
        'methods' => 'POST',
        'callback' => 'adminApproveDisapproveListingFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/toggle-featured', array(
        'methods' => 'POST',
        'callback' => 'toggleFeaturedFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/all-properties', array(
        'methods' => 'GET',
        'callback' => 'getAllPropertiesFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/expire-listing', array(
        'methods' => 'POST',
        'callback' => 'expireListingFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/sold-listing', array(
        'methods' => 'POST',
        'callback' => 'soldListingFunction',
        'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/pending-listing', array(
        'methods' => 'POST',
        'callback' => 'pendingListingFunction',
        'permission_callback' => '__return_true'
    ));
...
function checkUserVerificationFunction($request) {
    ...
    if ($enable_user_verification && $verification_required) {
        ...
        if (!houzez_is_admin() && !houzez_is_editor() && !is_exempt_from_verificationFunction($user_id)) {
...
function is_exempt_from_verificationFunction($user_id) {
...
function addPropertyWithAuthFunction() {
    $new_property['post_status']    = 'publish';
    ...
    if (!create_nonce_or_throw_errorFunction('verify_add_prop_nonce', 'add_property_nonce')) {
        return;
    }
...
function deletePropertyFunction() {
...
function uploadPropertyImageWithAuthFunction(){
    ...
    if (!create_nonce_or_throw_errorFunction('verify_nonce', 'verify_gallery_nonce')) {
        return;
      }
...
function deleteImageForPropertyFunction() {
    ...
    if (!create_nonce_or_throw_errorFunction('removeNonce', 'verify_gallery_nonce')) {
        return;
    }
...
function isFavPropertyFunction() {
...
function printPdfPropertyFunction() {
...
function getMyPropertiesFunction() {
    if (! is_user_logged_in() ) {
        $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
        wp_send_json($ajax_response, 403);
        return; 
...
function getAllPropertiesFunction() {
    if (! is_user_logged_in() ) {
        $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
        wp_send_json($ajax_response, 403);
        return; 
...
function getPropertyByPermalinkFunction($request) {
    ...
    queryPropertyByIdFunction($propertyId);

}
function getPropertyFunction($request) {
    ...
    sendPropertyJsonFunction($args);
}
function queryPropertyByIdFunction($propertyId) {   
    $args = array(
        'post_type'        =>  'property',
        'p'               =>  $propertyId,    
        'posts_per_page'   =>  1,
    ...
    //     appendPostImagesFunction($property);
    //     appendPostFeatureFunction($property);
    //     appendPostAddressFunction($property);
    //     appendPostAttrFunction($property);
    //     appendPostAttachmentsFunction($property);
...
function sendPropertyJsonFunction($args) {
...
function appendPostImagesFunction(&$property)
{
...
function appendPostAttachmentsFunction(&$property)
{
...
function appendPostFeatureFunction(&$property)
{
...
//     array('fields' => 'names')
//   );
    $property->property_features = getCurrentLanguageTermsOnlyFunction($property->ID, 'property_feature');
}

function appendPostAddressFunction(&$response)
{
...
function appendPostAttrFunction(&$response)
{
...
function getCurrentLanguageTermsOnlyFunction($postId, $term_name) {
    $current_lang = apply_filters( 'wpml_current_language', "en" );
...
function getPropertyByMetaKeyFunction($request) {
...
function adminApproveDisapproveListingFunction($request) {
    ...
    try {
        if ($action == 'publish' || $action == 'approve') {
            $result = approveListingFunction($listing_id);
        } elseif ($action == 'disapprove') {
            $result = disapprovalListingFunction($listing_id);
        } elseif ($action == 'on_hold') {
            $result = onHoldListingFunction($listing_id);
        } else {
//             $ajax_response = array('success' => false, 'reason' => 'Invalid action specified.');
//             return rest_ensure_response($ajax_response);
            return getAllPropertiesFunction();
        }
...
function approveListingFunction($listing_id) {
    $post_id = absint($listing_id);
...
function onHoldListingFunction($listing_id) {
    $post_id = absint($listing_id);
...
function disapprovalListingFunction($listing_id) {
    $post_id = absint($listing_id);
...
function expireListingFunction() {
    if (! is_user_logged_in() ) {
        $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
        wp_send_json($ajax_response, 403);
        return; 
...
function pendingListingFunction() {
    if (! is_user_logged_in() ) {
        $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
        wp_send_json($ajax_response, 403);
        return; 
...
function houzez_add_custom_endpointFunction() {
    add_rewrite_endpoint( 'print-property-pdf', EP_ROOT );
}
add_action( 'init', 'houzez_add_custom_endpointFunction' );

// ---------------
// Function to handle the custom 'print-property-pdf' endpoint.
function houzez_handle_custom_endpointFunction() {
...
add_action( 'template_redirect', 'houzez_handle_custom_endpointFunction' );

/*
 *   Backward compatibility in Case Houzez function doesn't work
*/

function houzi_create_printFunction () {
