...
add_action( 'rest_api_init', function () {
    ...
    register_rest_route( 'houzez-admin-api/v1', '/activities', array(
      'methods' => 'GET',
      'callback' => 'allActivitiesFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/leads', array(
      'methods' => 'GET',
      'callback' => 'allLeadsFunction',
      'permission_callback' => '__return_true'
    ));
    register_rest_route( 'houzez-admin-api/v1', '/add-lead', array(
      'methods' => 'POST',
      'callback' => 'addLeadFunction',
      'permission_callback' => '__return_true'
    ));
    register_rest_route( 'houzez-admin-api/v1', '/delete-lead', array(
      'methods' => 'POST',
      'callback' => 'deleteLeadFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/lead-details', array(
      'methods' => 'GET',
      'callback' => 'leadDetailsFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/lead-listing-viewed', array(
      'methods' => 'GET',
      'callback' => 'leadListingViewedFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/lead-saved-searches', array(
      'methods' => 'GET',
      'callback' => 'leadSavedSearchesFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/lead-notes', array(
      'methods' => 'GET',
      'callback' => 'leadNotesFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/enquiries', array(
      'methods' => 'GET',
      'callback' => 'getEnquiriesFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/all-enquiries', array(
      'methods' => 'GET',
      'callback' => 'allEnquiriesFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/enquiry-matched-listing', array(
      'methods' => 'GET',
      'callback' => 'enquiryMatchedListingFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/send-matched-listing-email', array(
      'methods' => 'POST',
      'callback' => 'sendMatchedListingEmailFunction',
      'permission_callback' => '__return_true'
    ));

    
    register_rest_route( 'houzez-admin-api/v1', '/enquiry-notes', array(
      'methods' => 'GET',
      'callback' => 'enquiryNotesFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/deals', array(
      'methods' => 'GET',
      'callback' => 'allDealsFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/add-deal', array(
      'methods' => 'POST',
      'callback' => 'addDealFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/update-deal-data', array(
      'methods' => 'POST',
      'callback' => 'updateDealDataFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/delete-deal', array(
      'methods' => 'POST',
      'callback' => 'deleteDealFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/add-crm-enquiry', array(
      'methods' => 'POST',
      'callback' => 'addCRMEnquiryFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/add-property-request', array(
      'methods' => 'POST',
      'callback' => 'addPropertyRequestFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/delete-crm-enquiry', array(
      'methods' => 'POST',
      'callback' => 'deleteCRMEnquiryFunction',
      'permission_callback' => '__return_true'
    ));
    
    register_rest_route( 'houzez-admin-api/v1', '/add-note', array(
      'methods' => 'POST',
      'callback' => 'addNoteFunction',
      'permission_callback' => '__return_true'
    ));
    register_rest_route( 'houzez-admin-api/v1', '/delete-note', array(
      'methods' => 'POST',
      'callback' => 'deleteNoteFunction',
      'permission_callback' => '__return_true'
    ));

    register_rest_route( 'houzez-admin-api/v1', '/user-crm-stats', array(
        'methods'             => 'GET',
        'callback'            => 'userCRMStatsFunction',
        'permission_callback' => '__return_true', 
    ));
    
  });

  function allActivitiesFunction(){
...
function leadListingViewedFunction() {
  //disable lightspeed caching
  do_action( 'litespeed_control_set_nocache', 'nocache due to logged in' );
  if (! is_user_logged_in() ) {
    $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
...
function leadSavedSearchesFunction() {
  //disable lightspeed caching
  do_action( 'litespeed_control_set_nocache', 'nocache due to logged in' );
  if (! is_user_logged_in() ) {
    $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
...
function enquiryMatchedListingFunction() {
  //disable lightspeed caching
  do_action( 'litespeed_control_set_nocache', 'nocache due to logged in' );
  if (! is_user_logged_in() ) {
    $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
...
function sendMatchedListingEmailFunction() {
  if (! is_user_logged_in() ) {
    $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
    wp_send_json($ajax_response, 403);
    return; 
...
function deleteDealFunction() {
  ...
  if (!create_nonce_or_throw_errorFunction('security', 'delete_deal_nonce')) {
    return;
  }
...
function deleteLeadFunction() {
  ...
  if (!create_nonce_or_throw_errorFunction('security', 'delete_lead_nonce')) {
    return;
  }
...
function addCRMEnquiryFunction() {
  //calls Houzez_Deals->add_new_deal();
  do_action("wp_ajax_crm_add_new_enquiry");
  
}
function addPropertyRequestFunction() {
...
function deleteCRMEnquiryFunction() {
...
function addNoteFunction() {
  ...
  // $_REQUEST['security'] = $nonce;
  if (!create_nonce_or_throw_errorFunction('security', 'note_add_nonce')) {
    return;
  }
...
function updateDealDataFunction() {
  if (! is_user_logged_in() ) {
    $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
    wp_send_json($ajax_response, 403);
    return; 
...
function userCRMStatsFunction( $request ) {
    do_action( 'litespeed_control_set_nocache', 'nocache due to logged in' );
