<?php
/**
 * Extends api for property post type, it appends many data that's not available in default wp-admin-api.
 *
 *
...
function houzez_property_rest_base_nameFunction( $string) {
...
add_filter( 'houzez_property_rest_base', 'houzez_property_rest_base_nameFunction', 10, 1 );


add_filter('rest_prepare_property', 'preparePropertyDataFunction', 10, 3);

function preparePropertyDataFunction($response, $post, $request)
{
  ...
  if($should_append_extra_data == true) {
    hm_postImagesFunction($response);
    hm_postFeatureFunction($response);
    hm_postAddressFunction($response);
    hm_postAttachmentsFunction($response);
  ...
  if (!empty($should_add_agent_agency_info) && $should_add_agent_agency_info == 'yes') {
    $response->data['property_meta']['agent_agency_info'] = property_agency_agent_infoFunction();
  }
...
function hm_postImagesFunction(&$response)
{
...
function hm_postAttachmentsFunction(&$response)
{
...
function hm_postFeatureFunction(&$response)
{
...
function hm_postAddressFunction(&$response)
{
...
function hm_postAttrFunction(&$response)
{
...
if(!function_exists('property_agency_agent_infoFunction')) {
  function property_agency_agent_infoFunction($is_top = true, $luxury = false) {
