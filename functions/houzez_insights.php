...
add_action('rest_api_init', function() {
    ...
    register_rest_route('houzez-insights/v1', '/system-check', [
        'methods' => 'GET',
        'callback' => 'houzez_insights_system_checkFunction',
        'permission_callback' => '__return_true'
    ]);
    
    register_rest_route('houzez-insights/v1', '/user-properties', [
        'methods' => 'GET',
        'callback' => 'houzez_get_user_propertiesFunction',
        'permission_callback' => 'is_user_logged_in'
    ]);
    
    register_rest_route('houzez-insights/v1', '/property-insights/(?P<id>\d+)', [
        'methods'  => 'GET',
        'callback' => 'houzez_get_real_property_insightsFunction',
        'permission_callback' => 'houzez_validate_property_accessFunction',
        'args' => [
            'id' => [
...
register_rest_route('houzez-insights/v1', '/user-insights', [
    'methods' => 'GET',
    'callback' => 'houzez_get_user_insightsFunction',
    'permission_callback' => 'is_user_logged_in',
    'args' => [
        'time_period' => [
            'default' => 'lastday',
 ...
 */
function houzez_insights_system_checkFunction() {
    return [
        'success' => true,
        'system' => [
...
//             $agent_ids = houzez_get_agency_agentsFunction($user_id);
//             $args_agents = [
//                 'post_type' => 'property',
...
//                 $agent_ids = houzez_get_agency_agentsFunction($agency_id);
//                 $args_agents = [
//                     'post_type' => 'property',
//                     'author__in' => $agent_ids,
//                     'posts_per_page' => -1,
...
function houzez_get_user_propertiesFunction(WP_REST_Request $request) {
    try {
...
if ( ! function_exists( 'houzez_get_agency_agentsFunction' ) ) {
    function houzez_get_agency_agentsFunction($agency_id) {
 ...
 */
function houzez_validate_property_accessFunction() {
    if (current_user_can('edit_others_posts')) {
        return true;
    }
 ...
 */
function houzez_get_property_insightsFunction($request) {
    try {
 ...
 */
function houzez_get_real_property_insightsFunction($request) {
    ...
    switch ($data_type) {
        case 'views':
            return [
                'total_views' => houzez_get_period_viewsFunction($stats, $time_period, $lastyear_views),
                'unique_views' => houzez_get_period_unique_viewsFunction($stats, $time_period, $lastyear_unique_views)
            ];
 ...
 */
function houzez_get_period_viewsFunction($stats, $time_period, $lastyear_value = 0) {
    $period_map = [
        'lastday' => 'lastday',
        'lastweek' => 'lastweek',
        'lastmonth' => 'lastmonth',
 ...
 */
function houzez_get_period_unique_viewsFunction($stats, $time_period, $lastyear_value = 0) {
    $period_map = [
        'lastday' => 'lastday',
        'lastweek' => 'lastweek',
        'lastmonth' => 'lastmonth',
 ...
 */
function houzez_get_user_insightsFunction($request) {
    try {
        $user_id = get_current_user_id();
        $time_period = sanitize_text_field($request->get_param('time_period'));
        $property_id = $request->get_param('property_id');
        
        $properties = houzez_get_user_propertiesFunction($request);
...
//         
        if ($is_single_property) {
    if (!houzez_user_can_access_propertyFunction($user_id, $property_id)) {
        return new WP_REST_Response([
            'success' => false,
            'error' => 'Invalid property ID or property not owned by user'
        ], 403);
...
function houzez_user_can_access_propertyFunction($user_id, $property_id) {
