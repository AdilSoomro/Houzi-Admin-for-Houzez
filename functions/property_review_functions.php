<?php
/**
 * Extends api for review. Exposes api to add review via admin api.
 *
 *
...
add_action('rest_api_init', function () {
    ...
    register_rest_route('houzez-admin-api/v1', '/add-review', array(
        'methods' => 'POST',
        'callback' => 'addReviewFunction',
        'permission_callback' => '__return_true'
    ));
    register_rest_route('houzez-admin-api/v1', '/report-content', array(
        'methods' => 'POST',
        'callback' => 'reportContentFunction',
        'permission_callback' => '__return_true'
    ));
    /// 
    register_rest_route('wp/v2', '/get-reviews', array(
        'methods' => 'GET',
        'callback' => 'getReviewsAPIFunction',
        'permission_callback' => '__return_true',
        'args' => array(
            'entity_type' => array(
                'required' => false,
    ...
    register_rest_route('houzez-reviews/v1', '/approve/(?P<id>\d+)', [
        'methods' => 'POST',
        'callback' => 'houzez_rest_approve_reviewFunction',
    ...
    register_rest_route('houzez-reviews/v1', '/reject/(?P<id>\d+)', [
        'methods' => 'POST',
        'callback' => 'houzez_rest_reject_reviewFunction',
        'permission_callback' => function () {
            return current_user_can('edit_posts');
        }
    ]);
    ...
    register_rest_route('houzez-reviews/v1', '/trash/(?P<id>\d+)', [
        'methods' => 'POST',
        'callback' => 'houzez_rest_trash_reviewFunction',
        'permission_callback' => function () {
    ...
    // Review Settings API
    register_rest_route('houzez-admin-api/v1', '/review-settings', [
        'methods' => 'GET',
        'callback' => 'houzez_get_review_settingsFunction',
        'permission_callback' => function () {
...
add_filter('rest_prepare_houzez_reviews', 'prepareReviewsDataFunction', 10, 3);

function prepareReviewsDataFunction($response, $post, $request)
{
...
function addReviewFunction()
{
    ...
    if (!create_nonce_or_throw_errorFunction('review-security', 'review-nonce')) {
        return;
    }
...
function reportContentFunction() {
    if (!is_user_logged_in()) {
        $ajax_response = array('success' => false, 'reason' => 'Please provide user auth.');
        wp_send_json($ajax_response, 403);
        return;
    ...
    if (!create_nonce_or_throw_errorFunction('report-security', 'report-nonce')) {
        return;
    }
...
////
function getReviewsAPIFunction($request)
{
    try {
...
function houzez_rest_trash_reviewFunction(WP_REST_Request $request)
{
...
function houzez_rest_approve_reviewFunction(WP_REST_Request $request)
{
    $review_id = $request->get_param('id');
 ...
 */
function houzez_rest_reject_reviewFunction(WP_REST_Request $request)
{
    $review_id = $request->get_param('id');
...
function houzez_get_review_settingsFunction()
{
