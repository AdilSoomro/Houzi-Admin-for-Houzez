...
add_action('rest_api_init', function () {
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'changeUserRoleFunction',
      'permission_callback' => '__return_true'
    )
  );
...
register_rest_route(
    ...
    array(
        'methods' => 'GET',
        'callback' => 'fetch_all_usersFunction',
	...
	/// New Post requests for user related actions
  	register_rest_route(
    	...
    	array(
      	'methods' => 'POST',
      	'callback' => 'houzez_handle_user_approval_apiFunction',
      	'permission_callback' => function () {
...
register_rest_route(
    ...
    array(
        'methods' => 'GET',
        'callback' => 'houzez_get_user_statusesFunction',
        'permission_callback' => function () {
            return current_user_can('edit_users');
        }
    )
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'tryPhoneLoginFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'resetUserPasswordFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'fixProfilePictureFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'editProfilePhotoFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'updatePasswordFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'deleteUserAccountFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'paymentStatusFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'GET',
      'callback' => 'proceedPaymentFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'proceedWithPaymentFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'makePropertyFeaturedFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'removeFromFeaturedFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route(
    ...
    array (
      'methods' => 'POST',
      'callback' => 'userCurrentPackageFunction',
      'permission_callback' => '__return_true'
    )
  );
  ...
  register_rest_route('contact-us/v1', 'send-message', array (
    'methods' => 'POST',
    'callback' => 'sendContactMailFunction',
    'permission_callback' => '__return_true',
    'args' => array (
      'name' => array (
        'required' => true,
  ...
  register_rest_route('houzez-admin-api/v1', '/save-searches-routine', array (
    'methods' => 'GET',
    'callback' => 'save_searches_routine_funcFunction',
    'permission_callback' => '__return_true',
  ));
...
/**
   ...
   */
function save_searches_routine_funcFunction() {
...
add_filter('jwt_auth_token_before_dispatch', 'add_user_info_to_loginFunction', 10, 2);

/**
 * Adds a website parameter to the auth.
 *
 */
function add_user_info_to_loginFunction($data, $user)
{
  $userID = $user->ID;
  $data['user_id'] = $userID;
...
function signInUserFunction()
{
  if (!create_nonce_or_throw_errorFunction('login_security', 'login_nonce')) {
    return;
  }
...
function tryPhoneLoginFunction()
{
  if (!create_nonce_or_throw_errorFunction('login_security', 'login_nonce')) {
    return;
  }
  ...
  if (strtolower($source) == 'phone') {
    ...
    if ($user) {
      ...
      doJWTAuthWithSecretFunction($user->user_login, $user->data->user_pass);
...
function socialSignOnFunction()
{
  ...
  if (strtolower($source) == 'phone') {
    ...
    if ($user) {
      ...
      doJWTAuthWithSecretFunction($username, $user->data->user_pass);
      return;
    }
  }
  ...
  if (email_exists($email)) {
    ...
    if ($user && wp_check_password($user_id_social, $user->data->user_pass, $user->ID)) {
      ...
      doJWTAuthFunction($email, $user_id_social);
      return;
    } else {
      doJWTAuthWithSecretFunction($email, $user->data->user_pass);
    }
  ...
  if ((strtolower($source) == 'apple') && empty($email)) {
    if (isset($_POST['display_name']) && !empty($_POST['display_name'])) {
      ...
      $username = convert_to_valid_usernameFunction($display_n);;
    } else {
      $username = convert_to_valid_usernameFunction(substr($user_id_social, 0, 15));
  ...
  if (username_exists($username)) {
    ...
    if ($user && wp_check_password($user_id_social, $user->data->user_pass, $user->ID)) {
      doJWTAuthFunction($username, $user_id_social);
      return;
    } else {
      doJWTAuthWithSecretFunction($username, $user->data->user_pass);
      return;
...
function checkPassWithSecretFunction($user_id, $candidate_password, $hashedpass)
{
...
function doJWTAuthWithSecretFunction($username, $password)
{
  ...
  doJWTAuthFunction($username, $secret . $password);
}
function doJWTAuthFunction($username, $password)
{
  $request = new WP_REST_Request('POST', '/jwt-auth/v1/token');
...
function signupUserFunction()
{
  ...
  if (!create_nonce_or_throw_errorFunction('houzez_register_security', 'houzez_register_nonce')) {
    return;
  }
  ...
  if (isset( $_POST['user_id_social']) && !empty( $_POST['user_id_social'])) {
    ...
    add_action('houzez_after_register', 'set_newly_created_social_idFunction');
  }
...
function set_newly_created_social_idFunction($user_id) {
  global $user_id_social_global;
  ...
  remove_action('houzez_after_register', 'set_newly_created_social_idFunction');
}

function adminAddUserFunction()
{
  if (!class_exists('Houzez_login_register')) {
    wp_send_json(array('error' => 'Houzez_login_register plugin dont exist'), 403);
    return;
...
function resetUserPasswordFunction()
{
  //create nonce for this request.
  // $nonce = wp_create_nonce('fave_resetpassword_nonce');
  // $_REQUEST['security'] = $nonce;

  if (!create_nonce_or_throw_errorFunction('security', 'fave_resetpassword_nonce')) {
    return;
  }
...
function updatePasswordFunction()
{
  ...
  if (!create_nonce_or_throw_errorFunction('houzez-security-pass', 'houzez_pass_ajax_nonce')) {
    return;
  }
...
function deleteUserAccountFunction()
{
...
function sendContactMailFunction(WP_REST_Request $request)
{
  $response = array(
    'status' => 304,
...
if (!function_exists('houzez_get_profile_thumb')) {
  function houzez_get_profile_thumbFunction($user_id = null)
  {
 ...
 * Change user role
*/
function changeUserRoleFunction($request){
    ...
    if (!create_nonce_or_throw_errorFunction('houzez-role-security-pass', 'houzez_role_pass_ajax_nonce')) {
        return;
    }
...
function editProfileFunction()
{
  ...
  if (!create_nonce_or_throw_errorFunction('houzez-security-profile', 'houzez_profile_ajax_nonce')) {
    return;
  }
...
/// If any error face then use this function (Incase of Roles)
...
function fixProfilePictureFunction()
{
  if (!is_user_logged_in()) {
    $ajax_response = array('success' => false, 'reason' => 'Please provide user auth.');
    wp_send_json($ajax_response, 403);
...
function editProfilePhotoFunction()
{
  if (!is_user_logged_in()) {
    $ajax_response = array('success' => false, 'reason' => 'Please provide user auth.');
    wp_send_json($ajax_response, 403);
  ...
  if (!create_nonce_or_throw_errorFunction('verify_nonce', 'houzez_upload_nonce')) {
    return;
  }
...
add_filter('check_password', 'check_with_hashed_passwordFunction', 10, 4);
...
function check_with_hashed_passwordFunction($check, $password, $hash, $user_id)
{
  $secret = '<6Bk-l1hPHr*?}V?v6~!-[cxvmjm4@9emdY+jezhH5z;5{rZiUFo|$W8X6llE_Hm';
 ...
 */
function is_sha1Function($str)
{
  return (bool) preg_match('/^[0-9a-f]{40}$/i', $str);
}

function paymentStatusFunction($request)
{
  if (!is_user_logged_in()) {
    $ajax_response = array('success' => false, 'reason' => 'Please provide user auth.');
    wp_send_json($ajax_response, 403);
...
function proceedPaymentFunction($request)
{
  // if (!is_user_logged_in() ) {
  //   $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
  //   wp_send_json($ajax_response, 403);
...
function proceedWithPaymentFunction($request)
{
  if (!is_user_logged_in()) {
    $ajax_response = array('success' => false, 'reason' => 'Please provide user auth.');
    wp_send_json($ajax_response, 403);
...
function makePropertyFeaturedFunction()
{
  if (!is_user_logged_in()) {
    $ajax_response = array('success' => false, 'reason' => 'Please provide user auth.');
    wp_send_json($ajax_response, 403);
...
function removeFromFeaturedFunction()
{
  if (!is_user_logged_in()) {
    $ajax_response = array('success' => false, 'reason' => 'Please provide user auth.');
    wp_send_json($ajax_response, 403);
...
function userCurrentPackageFunction()
{
  if (!is_user_logged_in()) {
    $ajax_response = array('success' => false, 'reason' => 'Please provide user auth.');
    wp_send_json($ajax_response, 403);
...
function convert_to_valid_usernameFunction($string) {
...
/////////// NEw
add_filter('rest_prepare_user', 'custom_add_user_fields_to_restFunction', 10, 3);

function custom_add_user_fields_to_restFunction($response, $user, $request)
{
...
function fetch_all_usersFunction($request)
{
...
function get_user_roles_for_apiFunction()
{
  global $wp_roles;
  $roles = array();
...
function houzez_handle_user_approval_apiFunction(WP_REST_Request $request)
{
...
function houzez_publish_user_postsFunction($user_id) {
...
function houzez_trash_user_postsFunction($user_id) {
...
function houzez_get_user_statusesFunction() {
    $statuses = array(
        array(
            'status' => 'pending',
            'status_code' => 0,
