...
add_action( 'rest_api_init', function () {
  register_rest_route( 'houzez-admin-api/v1', '/search-properties', array(
    'methods' => 'POST',
    'callback' => 'searchPropertiesFunction',
    'permission_callback' => '__return_true'
  ));

  register_rest_route( 'houzez-admin-api/v1', '/search-test', array(
    'methods' => 'POST',
    'callback' => 'searchPropertiesTestFunction',
    'permission_callback' => '__return_true'
  ));

  register_rest_route( 'houzez-admin-api/v1', '/get-property-detail', array(
    'methods' => 'GET',
    'callback' => 'getPropertDetailFunction',
    'permission_callback' => '__return_true'
  ));

  register_rest_route( 'houzez-admin-api/v1', '/similar-properties', array(
    'methods' => 'GET',
    'callback' => 'getSimilarPropertiesFunction',
    'permission_callback' => '__return_true'
  ));

  register_rest_route( 'houzez-admin-api/v1', '/favorite-properties', array(
    'methods' => 'GET',
    'callback' => 'getFavoritePropertiesFunction',
    'permission_callback' => '__return_true'
  ));

  register_rest_route( 'houzez-admin-api/v1', '/save-search', array(
    'methods' => 'POST',
    'callback' => 'saveSearchFunction',
    'permission_callback' => '__return_true'
  ));
  register_rest_route( 'houzez-admin-api/v1', '/saved-searches', array(
    'methods' => 'GET',
    'callback' => 'listSavedSearchesFunction',
    'permission_callback' => '__return_true'
  ));
  register_rest_route( 'houzez-admin-api/v1', '/view-saved-search', array(
    'methods' => 'POST',
    'callback' => 'viewSavedSearchFunction',
    'permission_callback' => '__return_true'
  ));
  register_rest_route( 'houzez-admin-api/v1', '/delete-saved-search', array(
    'methods' => 'POST',
    'callback' => 'deleteSearchFunction',
    'permission_callback' => '__return_true'
  ));
...
function getPropertDetailFunction(){
    $property_post_id   =   (isset($_GET['property_id']))?$_GET['property_id']:0;
    $prop               =   get_post( $property_post_id );

    if( isset($prop->ID) ){
        echo json_encode( array( 'getProperties' => true,  'property' => propertyNodeFunction( $prop ), 'original' => $prop ) );
    }else{
...
/* ******************************************************************************************************** */
    ...
    */
/* ******************************************************************************************************** */
function searchPropertiesFunction() {
    
    $query_args = setupSearchQueryFunction();
    
    queryPropertiesAndSendJSONFunction($query_args);
}
function searchPropertiesTestFunction() {
    
    $query_args = setupSearchQueryForTestingFunction();
    
    queryPropertiesAndSendJSONFunction($query_args);
}
function queryPropertiesAndSendJSONFunction($query_actual) {
    ...
    if( !empty( $keyword_filters ) ) {
        ...
        add_filter('posts_where', function($where) use ($search_queries) {
            global $wp_query;
            return custom_title_content_filterFunction($where, $wp_query, $search_queries);
        }, 10, 2);
    ...
    while( $query_args->have_posts() ):
        ...
        array_push($properties, propertyNodeFunction($property) );
...
function custom_title_content_filterFunction($where, $query, $search_queries) {
    global $wpdb;
...
function setupSearchQueryFunction() {
    $meta_query = array();
    $tax_query = array();
    $date_query = array();
    $allowed_html = array();
    ...
    $query_args = search_property_locationFunction( $query_args, $search_lat, $search_long, $search_radius, $use_radius, $search_location );
...
function setupSearchQueryForTestingFunction() {
    $meta_query = array();
    $tax_query = array();
    $date_query = array();
    $allowed_html = array();
...
function getSimilarPropertiesFunction() {
...
function propertyNodeFunction($property){
   ...
   if(!empty($property->thumbnail)) {
        ...
        $property->thumbnail = getThumbnailasPropertyImageFunction($post_id, $property);
    }
...
function getThumbnailasPropertyImageFunction(&$post_id, &$property) {
    $property->thumbnail = get_the_post_thumbnail_url( $post_id, 'houzez-property-thumb-image' );
    if(!empty($property->thumbnail)) {
        return $property->thumbnail;
    } else {
        if(!empty(getPropertyImageFirstIndexFunction($property)[0])){
            return getPropertyImageFirstIndexFunction($property)[0]; 
        }else {
            return getPropertyImageFirstIndexFunction($property)[1];
        }
    }
}
/// Return the first image URL from the property images
function getPropertyImageFirstIndexFunction($property): array
{
    $fullImage = null;
    $thumbImage = null;
...
function getFavoritePropertiesFunction() {
    ...
    if ( empty( $fav_ids ) || ! is_array( $fav_ids ) ) {
        ...
        foreach ($myposts as $post) : setup_postdata($post);
            array_push($properties, propertyNodeFunction($post) );
        endforeach;
        wp_reset_postdata();
...
function isFavoritePropertyFunction($prop_id) {
    if (!is_user_logged_in() ) {
        return false; 
    }
    $userID  = get_current_user_id();
...
function saveSearchFunction() {
    ...
    $query_args_orig = setupSearchQueryFunction();
    ...
    if (!create_nonce_or_throw_errorFunction('houzez_save_search_ajax', 'houzez-save-search-nounce')) {
        return;
    }
...
function listSavedSearchesFunction() {
    ...
    foreach ( $results as $houzez_search_data ) :
        ...
        $search_as_text = decodeParamsUtilFunction($search_args_decoded);
...
function viewSavedSearchFunction() {
    if (! is_user_logged_in() ) {
        $ajax_response = array( 'success' => false, 'reason' => 'Please provide user auth.' );
        wp_send_json($ajax_response, 403);
        return; 
...
function decodeParamsUtilFunction($search_args_decoded) {
    $meta_query     = array();
 ...
 */
function search_property_locationFunction($query_args, $latitude, $longitude, $radius, $use_radius, $search_location) {
